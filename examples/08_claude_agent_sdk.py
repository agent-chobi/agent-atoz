...
[기대 출력 예시] (출력은 실행마다 다르며 대략 이런 형태)
    === Claude Agent SDK: 내장 도구로 코드베이스 조사 ===

    examples/ 폴더에는 22개의 파이썬 파일이 있습니다:
    - 01_basic_message.py: Messages API 기본 호출
    - 02_streaming.py: 토큰 스트리밍
    ... (파일별 한 줄 요약이 이어짐)

    [완료] status=success

[흔한 에러]
    - SystemExit "claude-agent-sdk 가 설치되지 않았습니다" → pip install -r requirements.txt
    - CLINotFoundError / "Claude Code not found": Node.js 18+ 미설치 → Node.js 설치 후 재시도
...
async def main():
    ...
    options = ClaudeAgentOptions(
        ...
        permission_mode="default",               # "acceptEdits" 는 '파일 편집' 자동 승인용 — 이 예제는 읽기 전용이라 기본값이면 충분
