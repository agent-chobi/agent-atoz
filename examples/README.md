...
기본 모델은 **`claude-haiku-4-5`**(빠르고 저렴 — 학습에 충분)이며, 더 높은 품질이
필요하면 각 파일 상단의 `MODEL` 상수를 `claude-opus-4-8`로 바꾸세요.

> **모델 변경 예외 2건**: `08_claude_agent_sdk.py`는 별칭(`"haiku"` → `"opus"`),
> `14_subagents.py`는 provider 접두사 포함(`"anthropic:claude-opus-4-8"`) 형식을 사용합니다.
...
| `32_a2a_langgraph.py` | 한 파일에 서버/클라이언트 — **서버 모드 먼저**, 다른 터미널에서 `--client` |
...
| `27_guardrails.py` | 23. 가드레일 & 미들웨어 |
| `28_serve_fastapi.py` | 24. 배포와 운영 |
| `29_reliability_validation.py` | 25. 신뢰성 & 워크플로 검증 |
| `30_finetune_dataset.py` | 26. 파인튜닝으로 에이전트 강화 |
| `31_context_budget.py` | 08. 컨텍스트 엔지니어링 (실무 패턴) |
| `32_a2a_langgraph.py` | 12. A2A 프로토콜 (LangGraph 통합) |
