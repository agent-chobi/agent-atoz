...
[기대 출력 예시] (문구는 실행마다 다르며 대략 이런 형태 — handoff 여부가 성공 판단 기준)
    === 턴 1: Bob 을 불러 해적 말투로 인사시키기 ===
    (Alice 가 Bob 에게 handoff) 어이 항해사! 요호호, 나는 Bob 이다! ...

    === 턴 2: 이제 5 + 7 을 물어보면 Alice 로 handoff 된다 ===
    (Bob 이 Alice 에게 handoff) 5 + 7 = 12 입니다.
    ※ 턴 1은 해적 말투(Bob), 턴 2는 계산 결과 12(Alice)가 나오면 성공.
      같은 thread_id 라서 턴 2는 '마지막 활성 에이전트' Bob 에서 시작해 Alice 로 넘어간다.

[흔한 에러]
    - ImportError: No module named 'langgraph_swarm' → pip install -r requirements.txt 재실행
    - SystemExit "ANTHROPIC_API_KEY 가 없습니다" → .env 파일 확인
    - 멀티턴이 이어지지 않음: checkpointer 없이 compile 했거나 thread_id 가 턴마다 다름
"""
