...
MCP를 "쓸 줄 아는 것"과 "어디에 쓸지 아는 것"은 다릅니다. 도입 결정부터 서버 분할까지
네 단계로 정리합니다.

### 직접 함수 도구 vs MCP 서버화 — 결정 체크리스트

아래 항목 중 **하나라도 예**이면 MCP 서버화를 검토하고, **전부 아니오**면 직접 함수
도구로 두세요(과잉 표준화는 지연·운영 비용만 추가합니다).

- [ ] 이 도구를 **두 개 이상의 클라이언트**(LangGraph 앱 + Claude Desktop + IDE 등)에서 쓰는가?
- [ ] 도구를 만드는 팀과 쓰는 팀이 **다른가**(배포 주기를 분리해야 하는가)?
- [ ] 에이전트와 도구의 **언어/런타임이 다른가**(파이썬 에이전트 + TypeScript 연동 등)?
- [ ] 도구 접근에 **감사·인증을 한 지점에서** 걸어야 하는가?
- [ ] 이미 검증된 **기성 MCP 서버**(6절 레지스트리)가 존재하는가?

### 전송(transport) 선택

| 전송 | 구조 | 언제 |
|------|------|------|
| **stdio** | 클라이언트가 서버를 서브프로세스로 실행 | 로컬 개발·개인 도구·데스크톱 클라이언트. 인증 불필요, 가장 단순 |
| **streamable-http** | 독립 프로세스/호스트로 원격 서빙 | 팀 공유 서버, 컨테이너 배포, 인증(OAuth 등)이 필요한 프로덕션 |
| **HTTP + 게이트웨이** | 여러 MCP 서버 앞에 프록시를 세워 인증·감사·라우팅 집중 | 엔터프라이즈 — 서버가 수십 개로 늘고 조직 정책을 한 곳에서 걸어야 할 때 |

경험칙: **개발은 stdio로 시작**하고, 두 번째 소비자가 생기는 순간 streamable-http로,
서버 수가 한 자릿수를 넘고 보안 조직이 개입하는 순간 게이트웨이를 검토합니다.

### 도구 네이밍·스키마 설계 원칙

도구 이름과 설명은 사람이 아니라 **LLM이 읽는 API 문서**입니다.

- **이름은 `동사_명사`** — `search_docs`, `create_ticket`. 모호한 `run`, `process` 금지.
- **설명은 "언제 쓰는지"까지** — "티켓을 생성한다. 사용자가 버그를 신고하면 사용" 처럼
  선택 기준을 담아야 모델이 도구를 골라 쓸 수 있습니다.
- **파라미터는 좁게** — 자유 문자열 대신 enum/타입 제약. 파라미터가 5개를 넘으면 도구를
  쪼갤 신호입니다.
- **반환은 소화된 형태로** — 원본 JSON 덤프 대신 모델이 바로 쓸 요약/구조화 텍스트.
  반환이 크면 컨텍스트를 오염시킵니다([08장](08-context-engineering.md)).
- **부수효과 있는 도구는 이름에서 드러나게** — `delete_`, `send_` 접두사로 위험 도구를
  식별 가능하게 하면 [14장](14-permissions-security-hitl.md)의 승인 정책을 걸기 쉽습니다.

### 서버 분할 단위

- **도메인당 1서버**가 기본입니다(웹검색 서버 / DB 서버 / 사내 위키 서버). 4절의
  "서브에이전트마다 필요한 서버만" 배선이 자연스러워집니다.
- **권한 등급이 다르면 도메인이 같아도 분리**하세요 — 읽기 전용 조회 서버와 쓰기 가능
  서버를 나누면, 위험 도구만 승인 정책을 강하게 걸 수 있습니다.
- 반대로 **도구 1~2개짜리 서버를 남발하지 마세요** — 서버마다 연결·수명주기 관리
  비용이 붙습니다. 같은 도메인·같은 권한이면 한 서버에 모읍니다.
