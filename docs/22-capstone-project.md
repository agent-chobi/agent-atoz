...
## 설계 가이드 — 이 프로젝트를 실무로 확장할 때

각 Step을 프로덕션급으로 바꿀 때 어느 챕터의 설계 가이드를 적용할지의 매핑입니다.
확장 과제(위 note)를 하나의 그래프로 합친 뒤, 아래 순서대로 한 층씩 올리세요.

| Step | 프로덕션 전환 과제 | 적용할 설계 가이드 |
|------|--------------------|--------------------|
| 1 (도구) | 가짜 `web_search` → 실제 검색/MCP 서버, 도구 입력 스키마 고정 | [11장](11-mcp-integration.md) MCP, [18장](18-structured-output.md) strict tool use·실패 처리 계층 |
| 2 (supervisor) | 팀 그래프의 모듈화·스트리밍 UI, 워크플로우로 충분한 경로 회수 | [19장](19-workflow-patterns.md) 2축 매트릭스, [20장](20-langgraph-advanced.md) 서브그래프·스트리밍 |
| 3 (체크포인터) | SqliteSaver → Postgres, durability 모드 명시, 세션 경계·핸드오프 설계 | [20장](20-langgraph-advanced.md) durability 결정 트리, [17장](17-harness-engineering.md) 세션 경계·진행 상태 저장 |
| 4 (HITL) | 콘솔 y/n → 승인 큐+웹훅, 도구 위험도 분류, 감사 로그 | [14장](14-permissions-security-hitl.md) 정책 저장 위치·승인 큐·감사 스키마 |
| 5 (judge) | 1회 채점 → 골든셋 + CI 회귀 게이트, judge 검증 | [15장](15-evaluation-cost.md) 골든셋 추출·회귀 임계값·온/오프라인 배치 |
| 전체 | 트레이싱 부재 → 관측 스택 선정·마스킹·샘플링 | [13장](13-debugging-observability.md) 스택 결정 트리·계측 로드맵 |
| (선택) | 리서처에 사내 문서 검색 추가 | [21장](21-rag-agentic-rag.md) 벡터 DB·임베딩·전환 판단 |

최소 순서 제안 — 관측(13)부터 켜세요. 나머지 층의 개선은 전부 "실패를 볼 수 있는가"에
의존합니다. 그다음 위 매핑을 트래픽·위험도가 요구하는 것만 골라 적용합니다.

- [ ] 트레이싱이 켜져 있고, 실패 트레이스를 평가셋으로 승격하는 고리가 있는가 (13→15)
- [ ] 모든 도구에 위험도 등급이 붙어 있고, 미분류는 deny인가 (14)
- [ ] 프롬프트·모델 변경이 CI 회귀 게이트를 통과해야 배포되는가 (15)
- [ ] 장기 실행 시 세션 경계와 핸드오프 파일 규칙이 정의돼 있는가 (17)
- [ ] 단계 간 계약이 전부 구조화 출력으로 고정돼 있는가 (18·19)
