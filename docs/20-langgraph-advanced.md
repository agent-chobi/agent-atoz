...
## 설계 가이드 — 그래프 구조를 어떻게 짤 것인가

위 표의 선택지들을 실제 결정으로 옮기는 기준을 정리합니다.

### 서브그래프로 쪼개는 기준 — 셋 중 하나면 분리

- [ ] **팀 경계** — 다른 팀이 소유·배포하는 로직인가? 스키마 경계(방식 A/B)가 곧
      팀 간 계약이 되어, 내부 변경이 부모 그래프를 깨지 않습니다.
- [ ] **재사용** — 둘 이상의 부모 그래프가 같은 로직을 쓰는가? 복사 대신 서브그래프
      하나를 공유하면 수정이 한 곳으로 모입니다.
- [ ] **상태 격리** — 중간 산출물(검색 노트, 초안 이력)이 부모 컨텍스트를 오염시키는가?
      방식 A의 "내부 전용 키"가 [08장](08-context-engineering.md)의 격리를 공짜로 줍니다.

셋 다 아니면 쪼개지 마세요 — 노드 10개 이하 단일 그래프가 관측·디버깅 모두 쉽습니다.
쪼갤 때는 **공유 키를 최소로**(방식 A) 유지하고, 스키마가 2~3개 이상 어긋나기 시작하면
방식 B(변환 래퍼)로 전환하는 편이 경계가 명확합니다.

### 스트리밍 모드 — UI 요구에서 역산

| UI 요구 | stream_mode 조합 |
|---------|------------------|
| 챗봇 타이핑 효과만 | `messages` |
| + "지금 뭐 하는 중" 단계 표시 | `messages` + `custom` (노드에서 진행률 emit) |
| + 멀티에이전트 단계별 패널 | `updates` + `custom`, **`subgraphs=True` 필수** |
| 백엔드 배치 잡(UI 없음) | 스트리밍 생략, `invoke` + 트레이싱([13장](13-debugging-observability.md)) |

설계 순서가 중요합니다 — 프런트엔드가 보여줄 화면을 먼저 정하고 모드를 고르세요.
"일단 `values`로 다 받자"는 전체 상태 스냅샷을 매 스텝 직렬화하므로 상태가 크면
그 자체가 병목이 됩니다.
...
```mermaid
flowchart TD
    A{"중간에 죽으면<br/>재실행 비용이 큰가?<br/>(비싼 LLM 호출·긴 실행)"} -->|아니오| E["durability='exit'<br/>— 그냥 처음부터 다시"]
    A -->|예| B{"비가역 부수효과가 있는가?<br/>(결제·발송·배포)"}
    B -->|예| S["durability='sync'<br/>— 스텝 유실 = 이중 실행 위험"]
    B -->|아니오| AS["durability='async'<br/>— 성능·복구의 균형 기본값"]
```

`sync`를 골랐다면 그것으로 끝이 아닙니다 — §4의 결정성 규칙(부수효과를 task에 가두고
멱등하게)이 지켜져야 "재개 시 이중 결제" 같은 사고를 막습니다. durability는 저장
시점만 정할 뿐, 멱등성은 코드의 몫입니다.

### Functional API vs Graph API — 결정 규칙

§3의 비교표를 결정 규칙으로 줄이면:

- **순환(루프백 엣지)·동적 팬아웃(`Send`)·시각화가 필요하면 Graph API.** 이 셋은
  Functional API로 표현이 안 되거나 제한적입니다.
- **분기가 `if`/`for` 로 자연스럽게 읽히는 직선 흐름이면 Functional API.**
  [19장](19-workflow-patterns.md)의 chaining·routing·parallelization이 여기 해당합니다.
- **망설여지면 Functional API로 시작하세요.** 두 API는 같은 런타임(체크포인터·HITL·
  스트리밍)을 공유하고 섞어 쓸 수 있으므로, 복잡해진 부분만 나중에 StateGraph로
  승격하는 경로가 역방향(그래프를 함수로 축약)보다 훨씬 쌉니다.
